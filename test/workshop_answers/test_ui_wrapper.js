...
let helpers = require('../integration_tests/helper_methods');
...
var runTest = function(myPort, myCapabilities, myBefore, myAfter, myProxyUrl){
    describe("UI test to share an item ", function(){
        ...
        before(async function(){
            await helpers.startServer(myPort);
            if(myProxyUrl){
                helpers.request = requestPromise.defaults({'proxy':myProxyUrl});
            }        
            await myBefore(helpers);
        });
    
        after(async function(){
            await myAfter(helpers);
            if(driver && driver.quit) driver = driver.quit();
            server.server.close();        
        });
        
        helpers.longTest('should start the driver', async function(){
            driver = await helpers.startDriver(myCapabilities); 
            //With proxy
            // var capabilities = {
            //     'browserName': 'chrome',
            //     'proxy': {
